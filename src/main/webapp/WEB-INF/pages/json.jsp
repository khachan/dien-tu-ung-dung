...
<script
	...
	var searchCriteria;
		function show(items) {
			var row = $(".t-row")[0].cloneNode(true);
			var parent = $(".t-rows")[0];
			while($(".t-rows")[0].getElementsByTagName("tr").length > 0){
				parent.removeChild($(".t-rows")[0].getElementsByTagName("tr")[0]);
			}
			for(i = 0; i < items.length; i++){
				row = row.cloneNode(true);
				row.children[0].innerHTML = items[i].ten;
				row.children[1].innerHTML = items[i].chucNang;
				row.children[2].innerHTML = items[i].moTa;
				row.children[3].innerHTML = items[i].soLinhKien;
				row.children[4].getElementsByTagName("a")[0].href = items[i].maSo;
